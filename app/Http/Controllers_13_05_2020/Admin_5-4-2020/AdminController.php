...
class AdminController extends Controller
{
    ...
    public function index()
    {
       ...
       foreach($visitData as $key => $value){
        ...
        for($i = 0; $i < $counter; $i++){
            if(isset($data['visited'][$i]['transactionHistory'])){
                // array_push($data['revenueKey'],$data['visited'][$i]['transactionHistory']['subTotalRounded']);
                // array_push($data['revenueVal'],$data['visited'][$i]['transactionHistory']['createdDate']->get()->format('d-m-Y'));
